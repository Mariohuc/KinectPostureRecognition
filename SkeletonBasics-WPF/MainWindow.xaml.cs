...
namespace Microsoft.Samples.Kinect.SkeletonBasics
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void DrawBonesAndJoints(Skeleton skeleton, DrawingContext drawingContext)
        {
            ...
            //try
            //{
            //    string message = Estado.Text;
            //    if (message != null && message.Length > 0)
            //    {
            //        if (!anterior.Equals(message))
            //        {
            //            ipAdress = System.Net.IPAddress.Parse("127.0.0.1");
            //            ipEndpoint = new IPEndPoint(ipAdress, 19834);

            //            client = new Socket(ipAdress.AddressFamily, SocketType.Stream, ProtocolType.Tcp);
            //            client.Connect(ipEndpoint);
            //            byte[] sendmsg = Encoding.ASCII.GetBytes(message);
            //            byte[] msgLength = BitConverter.GetBytes(Convert.ToUInt32(sendmsg.Length));
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            if(!flag)
            {
                ...
                Thread myThread = new Thread(delegate ()
                {
                    ...
                    {
                        Estado.Dispatcher.Invoke(() => Estado.Text = "WAIT.....   "+(5 -s.Elapsed.Seconds-s.Elapsed.Milliseconds/1000.0));
                    }
                    s.Stop();
                    flag = !flag;
                    ...
                    {
                        Estado.Dispatcher.Invoke(() => Estado.Text = "GRABANDO...");
                    }
                    s.Stop();
                    flag = !flag;
                    Parte.Dispatcher.Invoke(() => File.WriteAllBytes(Parte.Text.ToUpper() + (new Random()).Next(1, 1000000000).ToString(), ObjectToByteArray(lista)));
                    numCount = 0;
                    s = new Stopwatch();
                    s.Start();
                    while (s.Elapsed < TimeSpan.FromSeconds(3))
                    {
                        Estado.Dispatcher.Invoke(() => Estado.Text = "FINISH...");
                    }
                });
                myThread.Start();
